...
class RegulaFalsiMethod:
    ...
    def solve(self,
        ...
        for iteration in range(iterations):
            a_func_val: float = self.function(equation, a)
            b_func_val: float = self.function(equation, b)

            c: float = ((a * b_func_val) - (b * a_func_val)) / (b_func_val - a_func_val)
            c_value: float = self.function(equation=equation, value=c)

            rounded_value: int or float = round(c_value, precision)

            print(
                f"Iteration {iteration}:\n  a: {a: .{precision}f} b: {b: .{precision}f} c: {c: .{precision}f}, c_value: {rounded_value}"
            )

            if (c_value > 0 and a_val > 0) or (c_value < 0 and a_val < 0):
                a = c
            else:
                b = c
