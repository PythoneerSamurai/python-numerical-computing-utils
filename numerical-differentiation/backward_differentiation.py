...
class BackwardDifferentiation:
    ...
    def __init__(self):
        # Stores backward difference table
        self.differences = []

    def difference_calculator(self, filtered_table_values: list):
        # Computes one level of backward differences
        differences = []
    ...
    def first_order_differentiation(self, step_size: int | float) -> float:
        ...
        for index, difference in enumerate(self.differences):
            diff += (1 / (index + 1)) * difference[-1]  # Use last element of each row
        return diff / step_size

    def second_order_differentiation(self, step_size: int | float) -> float:
        # Computes the second derivative using a weighted backward difference series
        diff = 0
        ...
        for index, difference in enumerate(self.differences[1:]):
            if index == 0 or index == 1:
                ...
                diff += fractional_coefficient * difference[-1]
    ...
    def third_order_differentiation(self, step_size: int | float) -> float:
        # Computes the third derivative using a weighted backward difference series
        diff = 0
    ...
    def solve(self, x_known: dict, x_diff: int | float, order: int) -> float | None:
        """
        Computes the first, second, or third derivative at a point using Newton's Backward Difference method.
        ...
        x_diff (int | float): The x-value at which the derivative is to be computed.
                              Must not be the first x-value in the table.
        ...
        assert x_diff != list(x_known.keys())[0]  # Cannot compute at first point (no backward data)

        x_diff_index = list(x_known.keys()).index(x_diff)
        # Take all values up to and including x_diff
        filtered_table_values = list(x_known.values())[: x_diff_index + 1]
